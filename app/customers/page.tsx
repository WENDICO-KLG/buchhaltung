...
import { CalendarRange, Mail, Pencil, Phone, Plus, ReceiptText, Trash2, Upload, Users } from "lucide-react";
import { WorkspaceShell } from "@/components/layout/workspace-shell";
import { createCustomer, deleteCustomer, getCustomers, updateCustomer, uploadCustomerLogo } from "@/lib/storage";
...
export default function CustomersPage() {
  ...
  async function uploadLogo(customerId: string, file: File) { try { const logoUrl = await uploadCustomerLogo(customerId, file); setCustomers((current) => current.map((customer) => customer.id === customerId ? { ...customer, logoUrl } : customer)); } catch (cause) { setError(cause instanceof Error ? cause.message : "Logo konnte nicht hochgeladen werden."); } }
