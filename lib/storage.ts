...
import type { CompanyProfile, Customer, Document, Expense, Invoice, MonthlyGoal, Prospect, WorkspaceFile, WorkspaceItem, WorkspaceModule } from "@/types";
...
export async function createInvoice(input: Omit<Invoice, "id" | "createdAt" | "updatedAt">) { const userId = await currentUserId(); const now = new Date().toISOString().slice(0, 10); const invoice: Invoice = { ...input, id: newId("invoice"), createdAt: now, updatedAt: now }; const { error } = await supabase.from("invoices").insert(invoiceToRow(invoice, userId)); if (error) throw databaseError(error); return invoice; }
export async function updateInvoice(id: string, changes: Partial<Invoice>) { const userId = await currentUserId(); const current = await getInvoice(id); if (!current) return undefined; const invoice: Invoice = { ...current, ...changes, updatedAt: new Date().toISOString().slice(0, 10) }; const { error } = await supabase.from("invoices").update(invoiceToRow(invoice, userId)).eq("id", id).eq("user_id", userId); if (error) throw databaseError(error); return invoice; }
...
export async function createProspect(input: Omit<Prospect, "id" | "createdAt" | "updatedAt">) { const userId = await currentUserId(); const now = new Date().toISOString().slice(0, 10); const prospect: Prospect = { ...input, id: newId("prospect"), createdAt: now, updatedAt: now }; const { error } = await supabase.from("prospects").insert({ id: prospect.id, user_id: userId, company_name: prospect.companyName, contact_name: prospect.contactName, email: prospect.email, phone: prospect.phone, source: prospect.source, status: prospect.status, last_contact_at: prospect.lastContactAt, next_task: prospect.nextTask, next_task_at: prospect.nextTaskAt, notes: prospect.notes, created_at: now, updated_at: now }); if (error) throw databaseError(error); return prospect; }
export async function updateProspect(id: string, changes: Partial<Prospect>) { const userId = await currentUserId(); const { error } = await supabase.from("prospects").update({ company_name: changes.companyName, contact_name: changes.contactName, email: changes.email, phone: changes.phone, source: changes.source, status: changes.status, last_contact_at: changes.lastContactAt, next_task: changes.nextTask, next_task_at: changes.nextTaskAt, notes: changes.notes, updated_at: new Date().toISOString().slice(0, 10) }).eq("id", id).eq("user_id", userId); if (error) throw databaseError(error); }
